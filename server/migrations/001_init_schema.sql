...
CREATE TABLE IF NOT EXISTS public.homeowners (
  ...
  homeComposition JSONB DEFAULT '{}',
  nationalId TEXT,
  workerInfo TEXT,
  specificDuties TEXT,
  workingHoursAndSchedule TEXT,
  numberOfWorkersNeeded INTEGER,
  ...
  bankDetails TEXT,
  religious TEXT,
  smokingDrinkingRestrictions TEXT,
  specificSkillsNeeded TEXT,
  profileComplete BOOLEAN DEFAULT FALSE,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW())
);
