...
class _ConversationsScreenState extends State<ConversationsScreen> {
  ...
  Future<void> _loadConversations() async {
    try {
      ...
      final conversations = await MessagingService.getConversations(widget.userId);
      setState(() {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        trailing: PopupMenuButton(
          ...
          onSelected: (value) => _handleConversationAction(conversation, value.toString()),
        ),
        onTap: () {
          Navigator.push(
            context,
  ...
  void _markConversationAsRead(Conversation conversation) async {
    try {
      await MessagingService.markConversationAsRead(conversation.id, widget.userId);
      _loadConversations();
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _showDeleteConversationDialog(Conversation conversation) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete this conversation with ${otherParticipant.name}?'),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                await MessagingService.deleteConversation(conversation.id, widget.userId);
                _loadConversations();
                ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _showBlockUserDialog(Conversation conversation) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to block ${otherParticipant.name}? They will no longer be able to send you messages.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Cancel'),
          ...
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(context);
              try {
                await MessagingService.blockUser(widget.userId, otherParticipant.id);
                _loadConversations();
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('${otherParticipant.name} has been blocked')),
                );
              } catch (e) {
                ScaffoldMessenger.of(context).showSnackBar(
