...
class JobService {
  ...
  static Future<void> _createRecurringJobs(Job parentJob) async {
    ...
    final endDate = parentJob.recurrenceEndDate ?? 
    ...
    while (nextDate.isBefore(endDate)) {
      ...
      switch (parentJob.recurrenceType) {
        case RecurrenceType.daily:
          nextDate = nextDate.add(Duration(days: parentJob.recurrenceInterval ?? 1));
          break;
        case RecurrenceType.weekly:
          nextDate = nextDate.add(Duration(days: 7 * (parentJob.recurrenceInterval ?? 1)));
          break;
        case RecurrenceType.biweekly:
          nextDate = nextDate.add(const Duration(days: 14));
          break;
        case RecurrenceType.monthly:
          nextDate = DateTime(nextDate.year, nextDate.month + (parentJob.recurrenceInterval ?? 1), nextDate.day);
          break;
        default:
          break;
      }
  ...
  static Future<List<Map<String, dynamic>>> _findMatchingWorkers(Job job) async {
    try {
      ...
      scoredWorkers.sort((a, b) => b['match_score'].compareTo(a['match_score']));
  ...
  static double _calculateMatchScore(Job job, Map<String, dynamic> worker) {
    ...
    final serviceCategories = List<String>.from(worker['service_categories'] ?? []);
    ...
    final jobDay = _getDayName(job.scheduledDate?.weekday ?? job.requestedDate.weekday);
  ...
  static Future<JobApplication> applyForJob({
    ...
    try {
      ...
      await SupabaseService.client.from('job_applications').insert(application.toJson());
  ...
  static Future<List<Job>> getHouseholdJobs(String householdId, {
    JobStatus? status,
    int limit = 50,
  }) async {
    try {
  ...
  // Get jobs for worker
  static Future<List<Job>> getWorkerJobs(String workerId, {
    JobStatus? status,
    int limit = 50,
  }) async {
    try {
  ...
  static Future<List<Job>> getAvailableJobs(String workerId, {
    ServiceType? serviceType,
    double? maxDistance,
    int limit = 20,
  }) async {
  ...
  static Future<List<JobApplication>> getJobApplications(String jobId) async {
    try {
      ...
      return response.map<JobApplication>((json) => JobApplication.fromJson(json)).toList();
    } catch (e) {
  ...
  static Future<List<ServicePackage>> getServicePackages({
    ...
    try {
      ...
      final response = await query;
      return response.map<ServicePackage>((json) => ServicePackage.fromJson(json)).toList();
    } catch (e) {
  ...
  // Get user ratings
  static Future<List<Rating>> getUserRatings(String userId, {
    int limit = 20,
  }) async {
    try {
      final response = await SupabaseService.client
  ...
  static Future<Map<String, dynamic>> getJobStatistics({
    ...
    try {
      ...
      final stats = {
        ...
        'completed_jobs': jobs.where((j) => j['status'] == JobStatus.completed.name).length,
        'pending_jobs': jobs.where((j) => j['status'] == JobStatus.pending.name).length,
        'cancelled_jobs': jobs.where((j) => j['status'] == JobStatus.cancelled.name).length,
        'total_earnings': jobs.fold<double>(0, (sum, job) => sum + (job['total_amount'] ?? 0.0)),
        'average_rating': 0.0,
      ...
      if (ratedJobs.isNotEmpty) {
        stats['average_rating'] = ratedJobs.fold<double>(0, (sum, job) => sum + job['rating']) / ratedJobs.length;
      }
