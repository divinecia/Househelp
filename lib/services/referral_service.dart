...
class ReferralService {
  ...
  static Future<bool> applyReferralCode(String userId, String code) async {
    try {
      ...
      await NotificationService.sendNotification(
        ...
        type: 'referral',
        data: {'referral_id': referralId, 'referred_user_id': userId},
      );
  ...
  static Future<void> completeReferral(String referralId) async {
    try {
      ...
      await NotificationService.sendNotification(
        ...
        message:
            'Your referral has been completed! Rewards have been credited.',
  ...
  static Future<void> _awardReferralRewards(
    String referrerId,
  ...
  static Future<Map<String, dynamic>> getReferralStatistics(
    String userId,
  ) async {
    try {
      ...
      final stats = {
        ...
        'completed_referrals': referrals
            .where((r) => r['status'] == ReferralStatus.completed.name)
            .length,
        'pending_referrals': referrals
            .where((r) => r['status'] == ReferralStatus.pending.name)
            .length,
        'total_earnings': 0.0,
        'by_month': <String, int>{},
      };
  ...
  static Future<void> _updateUserLoyaltyPoints(String userId) async {
    try {
      ...
      await _client.from('loyalty_points').upsert({
        'user_id': userId,
  ...
  static Future<List<LoyaltyTransaction>> getLoyaltyTransactions(
    String userId,
  ) async {
    try {
      final response = await _client
  ...
  static Future<bool> redeemReward(String userId, String rewardId) async {
    try {
      ...
      await NotificationService.sendNotification(
        ...
        data: {'reward_id': rewardId, 'redemption_id': redemptionId},
      );
  ...
  static Future<List<RewardRedemption>> getUserRedemptions(
    String userId,
  ) async {
    try {
      final response = await _client
      ...
      return response
          .map<RewardRedemption>((json) => RewardRedemption.fromJson(json))
          .toList();
    } catch (e) {
  ...
  static Future<void> awardJobCompletionPoints(
    String userId,
    double jobAmount,
  ) async {
    try {
  ...
  static Future<List<LoyaltyTier>> getLoyaltyTiers() async {
    try {
      ...
      return response
          .map<LoyaltyTier>((json) => LoyaltyTier.fromJson(json))
          .toList();
    } catch (e) {
  ...
  static Future<List<Promotion>> getActivePromotions() async {
    try {
      ...
      return response
          .map<Promotion>((json) => Promotion.fromJson(json))
          .toList();
    } catch (e) {
    ...
    try {
      ...
      if (userTier != null) {
        ...
        if (nextTier != userTier) {
          analytics['tier_progress'] =
              (userPoints - userTier.pointsRequired) /
      ...
      for (final transaction in transactions) {
        ...
        analytics['by_type'][type] =
            (analytics['by_type'][type] ?? 0) + transaction['points'];
      }
      ...
      for (final transaction in transactions) {
        ...
        final month = '${date.year}-${date.month.toString().padLeft(2, '0')}';
        analytics['monthly_earnings'][month] =
            (analytics['monthly_earnings'][month] ?? 0) + transaction['points'];
      }
  ...
  static Future<List<ReferralInvitation>> getReferralInvitations(
    String userId,
  ) async {
    try {
      final response = await _client
      ...
      return response
          .map<ReferralInvitation>((json) => ReferralInvitation.fromJson(json))
          .toList();
    } catch (e) {
