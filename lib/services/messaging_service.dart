...
class MessagingService {
  ...
  static Future<Message> sendMessage({
    ...
    try {
      ...
      await NotificationService.sendNotification(
        userId: recipientId,
        title: 'New Message',
        message: type == MessageType.text ? content : 'You received a ${type.name}',
        type: 'message',
        data: {
  ...
  static Future<Message> sendMediaMessage({
    ...
    try {
      ...
      final fileName = 'messages/${DateTime.now().millisecondsSinceEpoch}_${filePath.split('/').last}';
  ...
  static Stream<List<Conversation>> subscribeToConversations(String userId) {
    return _client
        ...
        .contains('participant_ids', [userId])
        .order('last_message_at', ascending: false)
        .map((data) => data.map<Conversation>((json) => Conversation.fromJson(json)).toList());
  }
  ...
  static Stream<List<TypingIndicator>> subscribeToTypingIndicators(String conversationId) {
    return _client
        .from('typing_indicators')
        .stream(primaryKey: ['conversation_id', 'user_id'])
        .eq('conversation_id', conversationId)
        .eq('is_typing', true)
        .map((data) => data.map<TypingIndicator>((json) => TypingIndicator.fromJson(json)).toList());
  }
  ...
  static Future<Map<String, dynamic>> getConversationAnalytics(String userId) async {
    try {
      ...
      final analytics = {
        ...
        'active_conversations': conversations.where((c) => c['is_active']).length,
        'total_messages_sent': messages.where((m) => m['sender_id'] == userId).length,
        'total_messages_received': messages.where((m) => m['recipient_id'] == userId).length,
      ...
      for (final message in messages) {
        ...
        analytics['message_types'][type] = (analytics['message_types'][type] ?? 0) + 1;
      }
  ...
  static Future<MessageDeliveryStatus> getMessageDeliveryStatus(String messageId) async {
    try {
      final response = await _client
