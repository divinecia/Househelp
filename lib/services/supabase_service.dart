...
class SupabaseService {
  ...
  static Future<bool> signInWithApple() async {
    try {
      final response = await client.auth.signInWithOAuth(OAuthProvider.apple);
      return response;
    } catch (e) {
  ...
  static Future<void> updateWorkerProfile({
    ...
    try {
      await client.from('worker_profiles').update(data).eq('user_id', userId);
    } catch (e) {
  ...
  static Future<List<Map<String, dynamic>>> searchWorkers({
    ...
    try {
      PostgrestFilterBuilder query = client.from('worker_profiles').select('''
            *,
            users!inner(*)
          ''');
  ...
  static Future<void> createHouseholdProfile({
    ...
    try {
      await client.from('household_profiles').insert({
        'user_id': userId,
        ...profileData,
      });
    } catch (e) {
  ...
  static Future<void> submitVerificationDocuments({
    ...
    try {
      ...
      throw Exception(
        'Failed to submit verification documents: ${e.toString()}',
      );
    }
  }
