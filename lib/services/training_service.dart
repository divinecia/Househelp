...
class TrainingService {
  ...
  static Future<List<TrainingCourse>> getAllCourses({
    ...
    try {
      ...
      return response.map<TrainingCourse>((json) => TrainingCourse.fromJson(json)).toList();
    } catch (e) {
  ...
  static Future<List<CourseModule>> getCourseModules(String courseId) async {
    try {
      ...
      return response.map<CourseModule>((json) => CourseModule.fromJson(json)).toList();
    } catch (e) {
  ...
  static Future<List<ModuleContent>> getModuleContent(String moduleId) async {
    try {
      ...
      return response.map<ModuleContent>((json) => ModuleContent.fromJson(json)).toList();
    } catch (e) {
  ...
  static Future<List<Enrollment>> getUserEnrollments(String userId) async {
    try {
      ...
      return response.map<Enrollment>((json) => Enrollment.fromJson(json)).toList();
    } catch (e) {
  ...
  static Future<void> _updateOverallProgress(String userId, String courseId) async {
    try {
      ...
      final progressPercentage = totalContent > 0 ? (completedContent / totalContent) * 100 : 0.0;
  ...
  static Future<Certificate> _generateCertificate(String userId, String courseId) async {
    try {
      ...
      await NotificationService.sendNotification(
        ...
        message: 'Your certificate has been generated and is ready for download.',
  ...
  static Future<List<Certificate>> getUserCertificates(String userId) async {
    try {
      ...
      return response.map<Certificate>((json) => Certificate.fromJson(json)).toList();
    } catch (e) {
  ...
  static Future<List<AssignmentSubmission>> getAssignmentSubmissions(String assignmentId) async {
    try {
      ...
      return response.map<AssignmentSubmission>((json) => AssignmentSubmission.fromJson(json)).toList();
    } catch (e) {
  ...
  static double _calculateQuizScore(Map<String, dynamic> quiz, Map<String, dynamic> answers) {
    try {
  ...
  // Get quiz attempts
  static Future<List<QuizAttempt>> getQuizAttempts(String userId, String quizId) async {
    try {
      final response = await _client
          .from('quiz_attempts')
          .select()
      ...
      return response.map<QuizAttempt>((json) => QuizAttempt.fromJson(json)).toList();
    } catch (e) {
  ...
  static Future<List<LearningPath>> getLearningPaths() async {
    try {
      ...
      return response.map<LearningPath>((json) => LearningPath.fromJson(json)).toList();
    } catch (e) {
  ...
  static Future<List<TrainingCourse>> getRecommendedCourses(String userId) async {
    try {
      ...
      final serviceCategories = List<String>.from(userProfile['service_categories'] ?? []);
  ...
  static Future<Map<String, dynamic>> getTrainingStatistics(String userId) async {
    try {
      ...
      final stats = {
        ...
        'completed_courses': enrollments.where((e) => e['status'] == EnrollmentStatus.completed.name).length,
        'in_progress_courses': enrollments.where((e) => e['status'] == EnrollmentStatus.active.name).length,
      ...
      if (enrollments.isNotEmpty) {
        final totalProgress = enrollments.fold<double>(0, (sum, enrollment) => sum + (enrollment['progress'] ?? 0.0));
  ...
  static Future<void> _updateCourseRating(String courseId) async {
    try {
      ...
      if (ratings.isNotEmpty) {
        final averageRating = ratings.fold<double>(0, (sum, rating) => sum + rating['rating']) / ratings.length;
  ...
  static Future<List<CourseRating>> getCourseRatings(String courseId) async {
    try {
      ...
      return response.map<CourseRating>((json) => CourseRating.fromJson(json)).toList();
    } catch (e) {
  ...
  static Future<List<TrainingCourse>> getInstructorCourses(String instructorId) async {
    try {
      final response = await _client
          .from('training_courses')
          .select()
  ...
  static Future<Map<String, dynamic>> getCourseAnalytics(String courseId) async {
    try {
      final enrollments = await _client
          .from('enrollments')
          .select()
      ...
      final completions = enrollments.where((e) => e['status'] == EnrollmentStatus.completed.name).length;
      final inProgress = enrollments.where((e) => e['status'] == EnrollmentStatus.active.name).length;
      ...
      final analytics = {
        ...
        'completion_rate': enrollments.isNotEmpty ? (completions / enrollments.length) * 100 : 0.0,
        'average_rating': ratings.isNotEmpty ? ratings.fold<double>(0, (sum, r) => sum + r['rating']) / ratings.length : 0.0,
        'total_revenue': enrollments.fold<double>(0, (sum, e) => sum + (e['payment_amount'] ?? 0.0)),
      ...
      for (final enrollment in enrollments) {
        ...
        analytics['monthly_enrollments'][month] = (analytics['monthly_enrollments'][month] ?? 0) + 1;
      }

      // Rating distribution
      for (final rating in ratings) {
        final score = (rating['rating'] as double).round();
        analytics['rating_distribution'][score] = (analytics['rating_distribution'][score] ?? 0) + 1;
      }
