...
class _EmergencyReportsWidgetState extends State<EmergencyReportsWidget> {
  ...
  Future<void> _updateReportStatus(String reportId, EmergencyStatus status) async {
    try {
  ...
  Widget _buildQuickStats() {
    ...
    final pendingReports = _reports.where((r) => r.status == EmergencyStatus.pending).length;
    ...
    return Row(
      children: [
        Expanded(
          child: _buildStatCard('Total', totalReports.toString(), AppConstants.primaryColor),
        ),
        const SizedBox(width: AppConstants.paddingSmall),
        Expanded(
          child: _buildStatCard('Pending', pendingReports.toString(), AppConstants.warningColor),
        ),
        const SizedBox(width: AppConstants.paddingSmall),
        Expanded(
          child: _buildStatCard('App Issues', appIssues.toString(), AppConstants.secondaryColor),
        ),
        const SizedBox(width: AppConstants.paddingSmall),
        Expanded(
          child: _buildStatCard('Criminal', criminalIssues.toString(), AppConstants.errorColor),
        ),
      ],
    );
  }
                  ...
                  Row(
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () => _callEmergencyNumber(report.emergencyContact!),
                      ),
                    ],
                  ),
                ...
                Row(
                  children: [
                    if (report.isAppIssue)
                      ElevatedButton.icon(
                        onPressed: () => _updateReportStatus(report.id, EmergencyStatus.inProgress),
                    ...
                    if (report.isCriminalIssue)
                      ElevatedButton.icon(
                        onPressed: () => _callEmergencyNumber(AppConstants.emergencyGenderViolence),
                    ...
                    const SizedBox(width: AppConstants.paddingSmall),
                    ElevatedButton.icon(
                      onPressed: () => _updateReportStatus(report.id, EmergencyStatus.resolved),
